    ...
    public void insertOrUpdateOrder(String orderId, String message, String date) {
        ...
        if (cursor != null) {
            ...
            if (cursor.getCount() > 0) {
                ...
                values.put(Order.COL_IS_PENDING, 0);
...
//                fromId = chatBean.getDetails().get(0).getFrom_id();
    ...
    public ArrayList<OrderHistory.Order_history> getOrderHis() {
        ...
        // descending order
        Cursor cursor = db.query(Order.TABLE, null, Order.COL_IS_PENDING + "=?", new String[]{"0"}, null, null, COL_ORDERID + " DESC");
    ...
    public ArrayList<OrderHistory.Order_history> getPendingOrder() {
        SQLiteDatabase db = this.getReadableDatabase();
        ArrayList<OrderHistory.Order_history> histories = new ArrayList<>();
        // descending order
        Cursor cursor = db.query(Order.TABLE, null, Order.COL_IS_PENDING + "=?", new String[]{"1"}, null, null, COL_ORDERID + " DESC");
    ...
    public void saveToNormalOrder(String orderId) {
    ...
    public void saveToPendingOrder(String orderId) {
        SQLiteDatabase db = this.getReadableDatabase();
        ContentValues val = new ContentValues();
        val.put(Order.COL_IS_PENDING, "1");
        db.update(Order.TABLE, val, Order.COL_ORDERID + "=? and " + Order.COL_IS_PENDING + "=?", new String[]{orderId, "0"});
        db.close();
    }
...
//    public ArrayList<PendingOrder.Pending_orders> getPendingOrder() {
//        SQLiteDatabase db = this.getReadableDatabase();
...
//    }
    ...
    public static class Order {
        ...
        public static final String COL_IS_PENDING = "is_pending"; // 0 - normal order, 1 - pending order

        public static final String QUERY = "CREATE TABLE " + TABLE + "(" + COL_ORDERID + " TEXT,"
                + COL_DATE + " TEXT," + COL_MESSAGE + " TEXT," + COL_IS_PENDING + " TEXT)";
    }
